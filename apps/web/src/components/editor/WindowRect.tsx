...
export const WindowRect = memo(function WindowRect({ rect, focused, label }: WindowRectProps) {
  return (
    <div
    ...
    >
      ...
      <div className="flex h-full w-full items-center justify-center">
        <div className="flex gap-1">
          {Array(focused ? DOT_COUNT : 1)
            .fill(0)
            .map((_, i) => (
              <div key={i} className="h-1.5 w-1.5 rounded-full bg-fd-muted-foreground/40" />
            ))}
        </div>
      </div>
    </div>
