...
function GeneralSection({
  ...
  return (
    <ConfigSection title="General">
      ...
      <SwitchRow
        label="Smart Gaps"
        checked={params.smartGaps}
        onCheckedChange={(v) => onUpdate({ smartGaps: v })}
      />
    </ConfigSection>
  );
}
...
function useLayoutRects(
  ...
  return useMemo(() => {
    ...
    switch (layoutType) {
      ...
      case "scroller":
        return calculateScrollerLayout(container, n, 0, gapParams, scrollerConfig, []);
      case "vertical-scroller":
        return calculateVerticalScrollerLayout(container, n, 0, gapParams, scrollerConfig, []);
      case "monocle":
